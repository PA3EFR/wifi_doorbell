 ...
 * 
 * Na ontvangst van "RING" stuurt de ontvanger een bevestiging "QSL"
 * terug naar de zender om aan te geven dat het signaal is ontvangen.
 * 
 * Functionaliteiten:
 ...
 * - Bevestiging (QSL) terugsturen naar zender
 * 
...
// IP van de zender (wordt bepaald bij ontvangst van eerste signaal)
IPAddress senderIP;
...
void loop() {
    ...
    if (packetSize) {
        ...
        // IP van afzender opslaan voor bevestiging
        ...
        if (strcmp(packetBuffer, doorbellPayload) == 0) {
            triggerBuzzer();
            sendAck();                                // Stuur bevestiging terug naar zender
        }
    }
...
void triggerBuzzer() {
    ...
    if (!isBuzzing) {
        ...
        Serial.println("Zoemer is al actief, signaal wordt nog steeds verwerkt");
    }
}

void sendAck() {
    // Stuur bevestiging "QSL" terug naar de zender
    Serial.println(">>> Bevestiging (QSL) versturen naar zender...");
    
    // Stuur QSL 2 keer voor redundantie
    for (int i = 0; i < 2; i++) {
        ...
        Serial.print(i + 1);
        Serial.print(" verzonden naar ");
        Serial.println(senderIP.toString());
        
        delay(20); // Korte pauze tussen pakketten
    }
